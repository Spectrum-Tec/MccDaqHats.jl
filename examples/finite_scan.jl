...
double calc_rms(double* data, uint8_t channel, uint8_t num_channels,
    ...
    value = 0.0
    for (i = 0:num_samples_per_channel -1)
...
main()
    ...
    uint8_t iepe_enable = true

    # Select an MCC172 HAT device to use.
    address = 0x00
    mcc172hatinformation = hatinfo("HAT_ID_MCC_172")
    addresspresent = false
    for i = length(mcc172hatinformation)
        if mcc172hatinformation[i].address == address
             addresspresent = true
        end
    end

    if addresspresent
        println("\nSelected MCC 172 device at $address \n")
    else
        println("\nMCC 172 at $address not present\n")
    end
