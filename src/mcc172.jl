...
# 1 if open, 0 if not open
	open = ccall((:mcc172_is_open, libdaqhats), 
		Cint, (UInt8,), address)
	return Bool(open)
end
...
Passing 0 for count will result in the LED blinking continuously until the board 
is reset or mcc172_blink_led() is called again with a non-zero value for count.
...
Returns:
Result code, RESULT_SUCCESS if successful.
version – Receives the firmware version. The version will be in BCD hexadecimal with 
the high byte as the major version and low byte as minor, i.e. 0x0103 is version 1.03.
"""
...
Returns:  Result code, RESULT_SUCCESS if successful.
"""
...
The sensitivity is specified in mV / mechanical unit. The default value when opening 
the library is 1000, resulting in no scaling of the input voltage.
...
This function will return the sample clock configuration and rate. If the clock is configured 
	for local or master source, then the rate will be the internally adjusted rate set by the 
		user. If the clock is configured for slave source, then the rate will be measured from 
		the master clock after the synchronization period has ended. The synchronization status 
		is also returned.
...
The clock source will be one of the following values:
	...
	SOURCE_MASTER = 1: The clock is generated on this MCC 172 and is shared as the master clock 
	for other MCC 172s.
...
"""
	mcc172_a_in_scan_read!(buffer::Vector{Float64}, address::Integer, samples_per_channel::Integer, mcc172_num_channels::Integer, timeout::Real)
